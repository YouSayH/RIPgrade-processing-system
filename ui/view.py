...
# import tkinter as tk
# from tkinter import ttk
# from tkinterdnd2 import DND_FILES # DND機能のためにインポート
# import os
# import sys
...
#         # --- D&Dとコマンドライン引数のハンドリング ---
#         self._setup_dnd() # このメソッドの中身を修正
#         self._handle_command_line_args()
...
#         self.cb_tsuki = ttk.Checkbutton(self.term_lf, text="通期", variable=self.vm.term_tsuki)
#         self.cb_tsuki.pack(side=tk.LEFT, padx=10, pady=5)
#         self._toggle_term_widgets()

#         action_frame = ttk.Frame(main_frame, padding=(0,10))
#         action_frame.pack(fill=tk.X, pady=10)
#         self.run_button = ttk.Button(action_frame, text="実行開始", command=self.vm.start_processing)
#         self.run_button.pack(pady=5)
#         s = ttk.Style()
#         s.configure("Accent.TButton", font=('Helvetica', 10, 'bold'), padding=6)
#         self.run_button.config(style="Accent.TButton")
        
#         progress_bar = ttk.Progressbar(action_frame, orient="horizontal", length=300, mode="determinate", variable=self.vm.progress_value)
#         progress_bar.pack(fill=tk.X, padx=5, pady=5)
...
#         self.vm.is_running.trace_add('write', self._toggle_run_button_state)
#         self.vm.status_text_log.trace_add('write', self._update_status_log)
#         self.vm.csv_path.trace_add('write', lambda *args: self._update_status_log(f"CSVファイル指定: {os.path.basename(self.vm.csv_path.get())}\n", add=True))
#         self.vm.excel_path.trace_add('write', lambda *args: self._update_status_log(f"Excelファイル指定: {os.path.basename(self.vm.excel_path.get())}\n", add=True))

#     def _setup_dnd(self):
#         """ドラッグ＆ドロップの有効化とイベントのバインド (修正後)"""
#         # このViewが配置されているトップレベルウィンドウを取得
#         toplevel = self.winfo_toplevel()
#         # トップレベルウィンドウにドロップターゲットを登録
#         # (main.pyでTkinterDnD.Tk()を使っているので、これでOK)
#         toplevel.drop_target_register(DND_FILES)
#         toplevel.dnd_bind('<<Drop>>', self._on_file_drop)
...
#     def _update_status_log(self, message="", add=False, *args):
#         # (このメソッドの中身は変更ありません)
#         self.status_text.config(state=tk.NORMAL)
#         if add:
